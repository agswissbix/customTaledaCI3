...
public function testsqlserver2()
{
    ...
    public function custom_winteler()
    {
        echo 'test winteler<br/>';
        $sql="SELECT  * FROM A1005";
        $result=$this->Sys_model->select($sql);
        var_dump($result); 
    }
    
    
    public function winteler()
    {
        $data['userid']=$this->get_userid();
        if($this->logged())
        {
            
            $data['content']=$this->load->view("sys/desktop/custom/winteler/winteler_menu",$data,true);
            echo $this->load->view("sys/desktop/custom/winteler/winteler_base",$data,true);
        }
        else
        {
            
            $data['content']=$this->load->view("sys/desktop/custom/winteler/winteler_login",$data,true);
            echo $this->load->view("sys/desktop/custom/winteler/winteler_base",$data,true);
        }
    }
    
    public function winteler_load_login()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_login",$data,true);
    }
    
    public function winteler_login()
    {
        //$this->session->set_userdata('username', 'a.galli');
        //$this->session->set_userdata('userid',1);
        //$this->winteler_load_menu();
        ...
        $connectionInfo = array( "Database"=>"winteler_meta", "UID"=>"sa", "PWD"=>"SB.s.s.21");
        $serverName = "WIGBSRV17";
        $connectionInfo = array( "Database"=>"Winteler_meta", "UID"=>"sa", "PWD"=>"Winteler,.-21");
        $conn = sqlsrv_connect( $serverName, $connectionInfo);

        if( $conn ) 
        {
            $sql="SELECT  * FROM TUSE WHERE FUNA='$utente' AND FPAS='$password'";
            ...
            {
                $userid=$user['FIUS'];
                $this->session->set_userdata('username', $utente);
                $this->session->set_userdata('userid',$userid);
                $this->winteler_load_menu();
            }
            else
            {
                $this->winteler_load_login();
            }
        }
        else
        {
    ...
    public function winteler_get_user($userid)
    {
        $serverName = "localhost"; //serverName\instanceName
        $connectionInfo = array( "Database"=>"winteler_meta", "UID"=>"sa", "PWD"=>"SB.s.s.21");
        $serverName = "WIGBSRV17";
        ...
        if( $conn ) 
        {
            $sql="SELECT  * FROM TUSE WHERE FIUS='$userid'";
            $stmt = sqlsrv_query($conn, $sql);
            $user=null;
            while($row = sqlsrv_fetch_array($stmt)) {
                $user=$row;
        ...
        $user_fsgl= $this->Sys_model->db_get_value('TUSE',"FSGL","FIUS='$userid'");
        $user['FSGL']=$user_fsgl;
        return $user;
    }
    
    public function winteler_get_users_sales()
    {
        
        $users= $this->Sys_model->db_get('TUSE',"*","(FIAL='1034' AND FIUS!='1067' AND FIUS!='1049' AND FIUS!='1044' AND FIUS!='1058' AND FIUS!='1059' AND FIUS!='1041') OR FIUS='1017' OR FIUS='1025' OR FIUS='1018' ","ORDER BY FSGL asc");
        return $users;
    }
    
    public function winteler_logout()
    {
        $this->session->sess_destroy();
        echo $this->winteler_load_login();
    }
    
    public function winteler_load_base($content='')
    {
    ...
    public function winteler_load_menu()
    {
        $data=array();
        $data['userid']=$this->get_userid();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_menu",$data,true);
       
    }
    
    public function winteler_load_conferma_salvataggio()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_conferma_salvataggio",$data,true);
       
    }
    
    public function winteler_load_barcode()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_barcode",$data,true);
    }
    
    public function winteler_load_barcode_test()
    {
        $data=array();
        //$data['files'] = scandir("../AdiServer/barcode"); 
        echo $this->load->view("sys/desktop/custom/winteler/winteler_barcode_test",$data,true);
    }
    
    public function winteler_load_quagga()
    {
        $data=array();
        
        echo $this->load->view("sys/desktop/custom/winteler/winteler_quagga",$data,true);
        //echo $this->load->view("sys/desktop/custom/winteler/winteler_base",$data,true);
    }
    
    public function winteler_load_notespese()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_notespese",$data,true);
    }
    
    public function winteler_load_serviceman_menu()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_serviceman_menu",$data,true);
    }
    
    public function winteler_load_serviceman_nuovo()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_serviceman_nuovo",$data,true);
    }
    
    public function winteler_load_serviceman_attesaconferma()
    {
        $data=array();
        $userid=$this->session->userdata('userid');
        $sql="SELECT * FROM VA1023 WHERE (F1154!='si' OR F1154 is null) AND FENA<>0"; 

        
        
        $rows=$this->Sys_model->select($sql);
        $data['servicemans']=$rows;
        echo $this->load->view("sys/desktop/custom/winteler/winteler_serviceman_attesaconferma",$data,true);
    }
    
   
    
    public function winteler_load_preventivocarrozzeria()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_preventivocarrozzeria",$data,true);
    }
    
    public function winteler_load_autonuove()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_autonuove",$data,true);
    }
    
    public function winteler_load_checklist_menu()
    {
         
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_checklist_menu",$data,true);
    }
    
    public function winteler_load_checklist($statoprocesso='Nuova',$fidd='')
    {
         
        $data=array();
        $data['venditori']=$this->winteler_get_users_sales();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_checklist",$data,true);
    }
    
    public function winteler_load_statistiche()
    {
         
        $data=array();
        $sql="SELECT F1,COUNT(*) AS counter FROM VA1021 WHERE FENA<>0 GROUP BY F1"; 
        
        
        $rows=$this->Sys_model->select($sql);
        
        foreach ($rows as $key => $row) {
            $rows[$key]['user']=$this->winteler_get_user($row['F1']);
        }
        $data['rows']=$rows;
        echo $this->load->view("sys/desktop/custom/winteler/winteler_statistiche",$data,true);
    }
    
    public function winteler_load_proveauto()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/winteler/winteler_proveauto",$data,true);
    }
    
    public function winteler_load_proveauto_nuova()
    {
        ...
        echo $this->load->view("sys/desktop/custom/winteler/winteler_proveauto_nuova",$data,true);
    }
    
    
    
    public function winteler_load_proveauto_aggiorna_precompilata($fidd)
    {
        $provaauto=$this->Sys_model->db_get_row('A1019','*',"FIDD='$fidd'");
            
        $data['fidd']=$fidd;
        $data['cognome']=$provaauto['F1073'];
        $data['nome']=$provaauto['F1074'];
        $data['email']=$provaauto['F1146'];
        $data['via']=$provaauto['F1134'];
        $data['cap']=$provaauto['F1041'];
        $data['citta']=$provaauto['F1042'];
        $data['telefono']=$provaauto['F1136'];
        $data['barcode']=$provaauto['F1028'];
        $data['telaio']=$provaauto['F1013'];
        $data['modello']=$provaauto['F1012'];
        $data['targa']=$provaauto['F1014'];
        $data['kmpartenza']=$provaauto['F1137'];
        $data['datapartenza']=$provaauto['F1138'];
        $data['orapartenza']=$provaauto['F1139'];
        $data['note']=$provaauto['F1033'];
        echo $this->load->view("sys/desktop/custom/winteler/winteler_proveauto_aggiorna_precompilata",$data,true);
    }
    
    public function winteler_load_proveauto_aggiorna_incorso($fidd)
    {
        $provaauto=$this->Sys_model->db_get_row('A1019','*',"FIDD='$fidd'");
        ...
        $data['orapartenza']=$provaauto['F1139'];
        $data['note']=$provaauto['F1033'];
        echo $this->load->view("sys/desktop/custom/winteler/winteler_proveauto_aggiorna_incorso",$data,true);
    }
    
    public function winteler_load_provaauto($statoprocesso='Nuova',$fidd='')
    {
        
        $data=array();
        $data['statoprocesso']=$statoprocesso;
        $data['fidd']=$fidd;
        $data['adiwid']='';
        ...
        if($provaauto!=null)
        {
            ...
            $data['kmarrivo']=$provaauto['F1140'];
            $data['dataarrivo']=$provaauto['F1141'];
            $data['oraarrivo']=$provaauto['F1142'];
        ...
        echo $this->load->view("sys/desktop/custom/winteler/winteler_provaauto",$data,true);
    }
    
    public function winteler_load_proveauto_precompilate($mostratutti='')
    {
        $data=array();
        $userid=$this->session->userdata('userid');
        if($mostratutti=='true')
        {
            $sql="SELECT * FROM VA1019 WHERE F1128='Precompilata' AND FENA<>0";
        }
        else
        {
            $sql="SELECT * FROM VA1019 WHERE F1128='Precompilata' AND FENA<>0 AND F1050='$userid'";
        }
        ...
        foreach ($rows as $key => $row) {
            $userid=$row['F1050'];
            $user=$this->winteler_get_user($userid);
            $rows[$key]['venditore']=$user['FSGL'];
            
        }
        $data['proveauto']=$rows;
        echo $this->load->view("sys/desktop/custom/winteler/winteler_proveauto_precompilate",$data,true);
    }
    
    public function winteler_load_proveauto_incorso($mostratutti='')
    {
        $data=array();
        $userid=$this->session->userdata('userid');
        if($mostratutti=='true')
        {
           $sql="SELECT * FROM VA1019 WHERE F1128='In corso' AND FENA<>0"; 
        }
        else
        {
           $sql="SELECT * FROM VA1019 WHERE F1128='In corso' AND FENA<>0 AND F1050='$userid'"; 
        }
        ...
        $data['proveauto']=$rows;
        echo $this->load->view("sys/desktop/custom/winteler/winteler_proveauto_incorso",$data,true);
    }
    
    
    public function winteler_get_dati_auto()
    {
        $data=array();
        $post=$_POST;
        ...
        $row['F1013']='';
        $row['F1012']='';
        if($this->isnotempty($barcode))
        {
            $row=$this->Sys_model->db_get_row("A1010","*","F1028 like '%$barcode%'");
        }
        if($this->isnotempty($telaio))
        {
            $row=$this->Sys_model->db_get_row("A1010","*","F1013 like '%$telaio%'");
        }
        if($this->isnotempty($row))
        {
            $data['result']='ok';
            $data['barcode']=$row['F1028'];
            $data['telaio']=$row['F1013'];
            $data['modello']=$row['F1012'];
        }
        else
        {
            $data['result']='ko';
    ...
    public function winteler_load_scheda_auto()
    {
        $data=array();
        $post=$_POST;
        $barcode_auto= $post['barcode'];
        $telaio_auto= $post['telaio'];
        if($this->isnotempty($telaio_auto))
        {
            $row=$this->Sys_model->db_get_row("A1010","*"," F1013 like '%$telaio_auto%'");
        }
        else
        {
            $row=$this->Sys_model->db_get_row("A1010","*","F1028 like '%$barcode_auto'");
        }
        
        if($row!=null)
        {
            $data['autotrovata']=true;
            $data['modello']=$row['F1012'];
            $data['libroauto']=$row['F1056'];
            $data['numerowb']=$row['F1057'];
            $data['telaio']=$row['F1013'];
            $data['designazione']=$row['F1058'];
            $data['barcode']=$row['F1028'];
            $FIDD=$row['FIDD'];
            $document_relative_path=$row['F10'];
            $document_relative_path= str_replace("/", "\\", $document_relative_path);
            $document_path="D:\Adiuto\\FTP\\".$document_relative_path;
            $temp_path="..\\AdiServer\\temp\\$FIDD.pdf";
            $temp_url=domain_url()."AdiServer/temp/$FIDD.pdf";
            //$temp_path="D:\Adiuto\xampp\htdocs\AdiServer\temp\\'.$FIDD.'.pdf"
            //$command='copy "'.$document_path.'" "D:\Adiuto\xampp\htdocs\AdiServer\temp\\'.$FIDD.'.pdf"';
            $command='copy "'.$document_path.'" "'.$temp_path.'"';
            exec($command);
            $data['document']=$temp_url;
            
            $data['attachments']=array();
            $attachments=$this->Sys_model->db_get('TLFD','*',"FIDD='$FIDD'");
            foreach ($attachments as $key => $attachment) {
                $document_relative_path=$attachment['FPTH'];
                $document_relative_path= str_replace("/", "\\", $document_relative_path);
                $document_path="D:\Adiuto\\FTP\\".$document_relative_path;
                $extension=substr( $document_path, strrpos( $document_path, '.' ) );
                $temp_filename=$FIDD.rand(1,999).$extension;
                $temp_path="..\\AdiServer\\temp\\$temp_filename";
                $command='copy "'.$document_path.'" "'.$temp_path.'"';
                exec($command);
                $temp_url=domain_url()."AdiServer/temp/$temp_filename";
                $data['attachments'][$key]['name']=$attachment['FNAME'];
                $data['attachments'][$key]['url']=$temp_url;
            }
            
            $data['linked_rows']=array();
            $linked_rows=$this->Sys_model->db_get('TLID','*',"FICD='$FIDD'");
            foreach ($linked_rows as $key => $linked_row) {
                $linked_row_FIDD=$linked_row['FIPD'];
                $linked_row_familyID= $this->Sys_model->db_get_value('TDOC','FIDF',"FIDD='$linked_row_FIDD'");
                $linked_row_family_description=$this->Sys_model->db_get_value('TFAM','FFAN',"FIDF='$linked_row_familyID'");
                $linked_document=$this->Sys_model->db_get_row("A".$linked_row_familyID,"*","FIDD='$linked_row_FIDD'");
                $document_relative_path=$linked_document['F10'];
                $document_relative_path= str_replace("/", "\\", $document_relative_path);
                $document_path="D:\Adiuto\\FTP\\".$document_relative_path;
                $extension=substr( $document_path, strrpos( $document_path, '.' ) );
                $temp_filename=$FIDD.rand(1,999).$extension;
                ...
                exec($command);
                $temp_url=domain_url()."AdiServer/temp/$temp_filename";
                $data['linked_rows'][$key]['name']=$linked_row_family_description." - $linked_row_FIDD";
                $data['linked_rows'][$key]['url']=$temp_url;
            }
            
        }
        else
        {
            $data['autotrovata']=false;
        }
        $data['barcode_auto']=$barcode_auto;
        
        echo $this->load->view("sys/desktop/custom/winteler/winteler_scheda_auto",$data,true);
    }
    
    public function winteler_salva_notespese()
    {
        $post=$_POST;
        $tipo=$post['tipo'];
        ...
        $data=date('Y#m#d');
        $userid= $this->session->userdata('userid');
        $filename="NS#$userid#$data#$tipo#$importo#$note.jpg";
        $note=$post['note'];
        $files=$_FILES['file'];
        if(count($files)>0)
        {
            move_uploaded_file($files['tmp_name'][0], "../AdiServer/notespese/$filename");
            $command='move "..\\AdiServer\\notespese\\'.$filename.'" "D:\Adiuto\Immission\Notespese"';
            $this->esegui($command);
        }
    ...
    public function winteler_salva_serviceman()
    {
        $post=$_POST;
        $userid=$this->session->userdata('userid');

        $testo= json_encode($post);
        
        $this->salva_log($userid,"winteler_salva_serviceman-post-".$testo);
        
        $adiwid=time ( )."-".rand(1000,9999);
        $utente=$this->session->userdata('userid');
        
        $cognome=replace_singlequote($post['cognome']);
        $nome=replace_singlequote($post['nome']);
        $email=replace_singlequote($post['email']);
        $telefono=replace_singlequote($post['telefono']);
        
        $telaio=replace_singlequote($post['telaio']);
        $modello=replace_singlequote($post['modello']);
        $targa=replace_singlequote($post['targa']);
        
        $nomeutente=replace_singlequote($post['nomeutente']);
        
        $data_originale=$post['data'];        
        $data=date("Ymd",strtotime($data_originale));
        
        
        $ora=replace_singlequote($post['ora']);
        if(!isempty($ora))
        {
        ...
        $sql="
        INSERT INTO T_SERVICEMAN
        (F1144adiwid,F1013telaio,F1012modello,F1014targa,F1073cognome,F1074nome,F1146email,F1136telefono,F1127utenteinserimento,F1156nomeutente,F1004data,F1157ora)
        VALUES
        ('$adiwid','$telaio','$modello','$targa','$cognome','$nome','$email','$telefono','$utente','$nomeutente','$data','$ora')
        ";
        $this->salva_log($userid,"winteler_salva_serviceman-".$sql);
        $this->Sys_model->execute_query($sql);
        ...
        if(array_key_exists('file', $_FILES))
        {
            ...
            if(count($files)>0)
            {
                    ...
                    foreach ($files['tmp_name'] as $key => $file) {
                        $filename="A#$adiwid#$counter.jpg";
                        move_uploaded_file($file, "../AdiServer/serviceman/$filename");
                        $width=500;
                        $height=500;
                        $command='cd ./tools/ImageMagick/ && convert "../../../AdiServer/Serviceman/'.$filename.'" -resize '.$width.'x'.$height.' "../../../AdiServer/Serviceman/'.$filename.'"';
                        $this->esegui($command);
                        $command='move "..\\AdiServer\\serviceman\\'.$filename.'" "D:\Adiuto\Immission\Serviceman"';
                        $this->esegui($command);
                        $counter++;
                    }
    ...
    public function winteler_salva_preventivocarrozzeria()
    {
        $post=$_POST;
        $userid=$this->session->userdata('userid');
        ...
        $this->salva_log($userid,"winteler_salva_preventivocarrozzeria-post-".$testo);
        ...
        $sql="
        INSERT INTO T_PREVENTIVOCARROZZERIA
        (F1144adiwid,F1013telaio,F1012modello,F1014targa,F1127utenteinserimento,F1156nomeutente)
        VALUES
        ('$adiwid','$telaio','$modello','$targa','$utente','$nomeutente')
        ";
        $this->salva_log($userid,"winteler_salva_preventivocarrozzeria-".$sql);
        $this->Sys_model->execute_query($sql);
        ...
        if(array_key_exists('file', $_FILES))
        {
            ...
            if(count($files)>0)
            {
                    ...
                    foreach ($files['tmp_name'] as $key => $file) {
                        $filename="A#$adiwid#$counter.jpg";
                        move_uploaded_file($file, "../AdiServer/preventivocarrozzeria/$filename");
                        $width=500;
                        $height=500;
                        $command='cd ./tools/ImageMagick/ && convert "../../../AdiServer/preventivocarrozzeria/'.$filename.'" -resize '.$width.'x'.$height.' "../../../AdiServer/preventivocarrozzeria/'.$filename.'"';
                        $this->esegui($command);
                        $command='move "..\\AdiServer\\preventivocarrozzeria\\'.$filename.'" "D:\Adiuto\Immission\preventivocarrozzeria"';
                        $this->esegui($command);
                        $counter++;
                    }
    ...
    public function winteler_salva_autonuova()
    {
        $post=$_POST;
        $userid=$this->session->userdata('userid');
        ...
        $this->salva_log($userid,"winteler_salva_autonuova-post-".$testo);
        ...
        $sql="
        INSERT INTO T_FORMAUTONUOVE
        (F1144adiwid,F1013telaio,F1012modello,F1127utenteinserimento,F1156nomeutente)
        VALUES
        ('$adiwid','$telaio','$modello','$utente','$nomeutente')
        ";
        $this->salva_log($userid,"winteler_salva_autonuova-".$sql);
        $this->Sys_model->execute_query($sql);
        ...
        if(array_key_exists('file', $_FILES))
        {
            ...
            if(count($files)>0)
            {
                    ...
                    foreach ($files['tmp_name'] as $key => $file) {
                        $filename="A#$adiwid#$counter.jpg";
                        move_uploaded_file($file, "../AdiServer/autonuova/$filename");
                        $width=500;
                        $height=500;
                        $command='cd ./tools/ImageMagick/ && convert "../../../AdiServer/autonuova/'.$filename.'" -resize '.$width.'x'.$height.' "../../../AdiServer/autonuova/'.$filename.'"';
                        $this->esegui($command);
                        $command='move "..\\AdiServer\\autonuova\\'.$filename.'" "D:\Adiuto\Immission\autonuova"';
                        $this->esegui($command);
                        $counter++;
                    }
    ...
    public function winteler_test_barcode()
    {
        $this->load->view("sys/desktop/custom/winteler/winteler_barcode_1");
    }
    
    public function winteler_salva_prove()
    {
        $post=$_POST;
        $userid=$this->session->userdata('userid');
        $testo= json_encode($post);
        $this->salva_log($userid,"winteler_salva_prove-post-".$testo);
                
        if(array_key_exists('fidd', $post))
        {
            
            $statoprocesso=$post['statoprocesso'];
            $statoprocesso= str_replace("%20", " ", $statoprocesso);
        }
        else
        {
            $statoprocesso="Nuova";
        ...
        if(array_key_exists('file', $_FILES))
        {
            ...
            if(count($files)>0)
            {
                if(array_key_exists('patente', $files['tmp_name']))
                {
                    foreach ($files['tmp_name']['patente'] as $key => $file) {
                        $filename="A#$adiwid#Patente#$counter.jpg";
                        move_uploaded_file($file, "../AdiServer/Prove auto - allegati/$filename");
                        $width=500;
                        $height=500;
                        $command='cd ./tools/ImageMagick/ && convert "../../../AdiServer/Prove auto - allegati/'.$filename.'" -resize '.$width.'x'.$height.' "../../../AdiServer/Prove auto - allegati/'.$filename.'"';
                        $this->esegui($command);
                        $command='move "..\\AdiServer\\Prove auto - allegati\\'.$filename.'" "D:\Adiuto\Immission\Prove auto - allegati"';
                        $this->esegui($command);
                        $counter++;
                    }

                }
                if(array_key_exists('autoprima', $files['tmp_name']))
                {
                    foreach ($files['tmp_name']['autoprima'] as $key => $file) {
                        $filename="A#$adiwid#Auto prima#$counter.jpg";
                        move_uploaded_file($file, "../AdiServer/Prove auto - allegati/$filename");
                        $width=500;
                        $height=500;
                        $command='cd ./tools/ImageMagick/ && convert "../../../AdiServer/Prove auto - allegati/'.$filename.'" -resize '.$width.'x'.$height.' "../../../AdiServer/Prove auto - allegati/'.$filename.'"';
                ...
                if(array_key_exists('autodopo', $files['tmp_name']))
                {
                   
                    foreach ($files['tmp_name']['autodopo'] as $key => $file) {
                        $filename="A#$adiwid#Auto dopo#$counter.jpg";
                        move_uploaded_file($file, "../AdiServer/Prove auto - allegati/$filename");
                        $width=500;
                        $height=500;
                        $command='cd ./tools/ImageMagick/ && convert "../../../AdiServer/Prove auto - allegati/'.$filename.'" -resize '.$width.'x'.$height.' "../../../AdiServer/Prove auto - allegati/'.$filename.'"';
        ...
        $barcode=replace_singlequote($post['barcode']);
        $telaio=replace_singlequote($post['telaio']);
        $modello=replace_singlequote($post['modello']);
        $targa=$post['targa'];
        ...
        $nome=replace_singlequote($post['nome']);
        $email=replace_singlequote($post['email']);
        $via=replace_singlequote($post['via']);
        $cap=replace_singlequote($post['cap']);
        $citta=replace_singlequote($post['citta']);
        $telefono=replace_singlequote($post['telefono']);
        
        $kmpartenza=replace_singlequote($post['kmpartenza']);
        $datapartenza=replace_singlequote($post['datapartenza']);
        if(!isempty($datapartenza))
        {
            $datapartenza=date("Ymd",strtotime($datapartenza));
        }
        $orapartenza=replace_singlequote($post['orapartenza']);
        if(!isempty($orapartenza))
        {
            $orapartenza=date("H:i",strtotime($orapartenza));
        }
        
        $kmarrivo=replace_singlequote($post['kmarrivo']);
        $dataarrivo=replace_singlequote($post['dataarrivo']);
        if(!isempty($dataarrivo))
        {
            $dataarrivo=date("Ymd",strtotime($dataarrivo));
        }
        $oraarrivo=$post['oraarrivo'];
        if(!isempty($oraarrivo))
        {
            $oraarrivo=date("H:i",strtotime($oraarrivo));
        }
        
        $note=replace_singlequote($post['note']);
        
        if($statoprocesso=='Nuova')
        {
            $userid=$this->session->userdata('userid');
            $venditore=$this->session->userdata('userid');
            
            $statoprocesso='In corso';
            $precompilata=$post['precompilata'];
            if($precompilata=='true')
            {
                $statoprocesso='Precompilata';
            }
            $sql="
            INSERT INTO T_PROVEAUTO
            (F1144adiwid,F1028barcode,F1013telaio,F1012modello,F1014targa,F1073cognome,F1074nome,F1146email,F1134via,F1041cap,F1042citta,F1136telefono,F1137kmpartenza,F1138datapartenza,F1139orapartenza,F1140kmarrivo,F1141dataarrivo,F1142oraarrivo,F1033note,F1050venditore,F1128statoprocesso)
            VALUES
            ('$adiwid','$barcode','$telaio','$modello','$targa','$cognome','$nome','$email','$via','$cap','$citta','$telefono','$kmpartenza','$datapartenza','$orapartenza','$kmarrivo','$dataarrivo','$oraarrivo','$note','$venditore','$statoprocesso')
            ";
        }
        else
        {
            if($statoprocesso=='Precompilata')
            {
                $statoprocesso='In corso';
                $sql="
                    UPDATE A1019
                    SET F1146='$email',F1012='$modello',F1013='$telaio',F1014='$targa',F1028='$barcode',F1033='$note',F1041='$cap',F1042='$citta',F1073='$cognome',F1074='$nome',F1134='$via',F1136='$telefono',F1137='$kmpartenza',F1138='$datapartenza',F1139='$orapartenza',F1128='In corso'
                    WHERE FIDD='$fidd'
                    ";
                $sql="
                INSERT INTO T_PROVEAUTO
            ...
            {
                if($statoprocesso=='In corso')
                {
    
                    
                    if(($this->isnotempty($dataarrivo))&&($this->isnotempty($oraarrivo)))
                    {
                        $statoprocesso='Conclusa';
                    }
                    $sql="
                        UPDATE A1019
                        SET F1146='$email',F1012='$modello',F1013='$telaio',F1014='$targa',F1028='$barcode',F1033='$note',F1041='$cap',F1042='$citta',F1073='$cognome',F1074='$nome',F1134='$via',F1136='$telefono',F1137='$kmpartenza',F1138='$datapartenza',F1139='$orapartenza',F1140='$kmarrivo',F1141='$dataarrivo',F1142='$oraarrivo',F1128='$statoprocesso'
                        WHERE FIDD='$fidd'
                    ";
        ...
        $this->salva_log($userid,"winteler_salva_prove-sql-".$sql);
        $this->Sys_model->execute_query($sql);
        //$filename="PA#$cognome#$nome#$email#$via#$cap#$citta#$telefono#$barcode#$telaio#$modello#$targa#$kmpartenza#$datapartenza#$orapartenza#$adiwid.txt";
        //fopen("../AdiServer/Prove auto/$filename", "w");
        //$command='move "..\\AdiServer\\Prove auto\\'.$filename.'" "D:\Adiuto\Immission\Prove auto"';
        //$this->esegui($command);
    ...
    public function winteler_salva_proveauto_aggiorna_incorso()
    {
        $post=$_POST;
        
        $fidd=$post['fidd'];
        
        $adiwid=time ( )."-".rand(0,1000);
        
        
        $sql="UPDATE A1019 SET F1140='$kmarrivo',F1141='$dataarrivo',F1142='$oraarrivo',F1128='Conclusa' WHERE FIDD='$fidd'";
        $this->Sys_model->execute_query($sql);
        
        echo 'SALVATO';
    }
    
    public function winteler_salva_proveauto_aggiorna_conclusa()
    {
        $post=$_POST;
        
        $fidd=$post['fidd'];
        $kmarrivo=$post['kmarrivo'];
        $dataarrivo=$post['dataarrivo'];
        $oraarrivo=$post['oraarrivo'];
        ...
        if(array_key_exists('file', $_FILES))
        {
            ...
            if(count($files)>0)
            {
                if(array_key_exists('autodopo', $files['tmp_name']))
                {
                    foreach ($files['tmp_name']['autodopo'] as $key => $file) {
                        $filename="A#$adiwid#Auto dopo#$counter.jpg";
                        move_uploaded_file($file, "../AdiServer/allegati/$filename");
                        $command='move "..\\AdiServer\\allegati\\'.$filename.'" "D:\Adiuto\Immission\Allegati"';
                        $this->esegui($command);
                        $counter++;
                    }
    ...
    public function test_quagga()
    {
        $content=$this->load->view("sys/desktop/custom/winteler/test_quagga",array(),true);
        $this->winteler_load_base($content);
    }
    
    
    public function winteler_caricabarcode()
    {
        $post=$_POST;
        $files=$_FILES['file'];
        $filename=rand(100,1000).".jpg";
        if(count($files)>0)
        {
            move_uploaded_file($files['tmp_name'][0], "../AdiServer/barcode/$filename");
        }
        //$command='"D:\Adiuto\ZBar\bin\zbarimg.exe" --raw -q D:\Adiuto\xampp\htdocs\AdiServer\barcode\\'.$filename.'"';
        //exec($command,$output);
        $command='"./tools/bardecode/bardecode.exe" "..\\AdiServer\\barcode\\'.$filename.'" -K 1RmK2jEXcERMF6F71C0g921q6H1RmKLM';
        exec($command,$output);
        $barcode="Barcode non riconosciuto";
        if(count($output)>0)
        {
            $barcode=$output[0];
        }
    ...
    public function winteler_get_datiauto_by_barcode($barcode)
    {
        $barcode_auto= $post['barcode'];
        $telaio_auto= $post['telaio'];
        if($this->isnotempty($telaio_auto))
        {
            $row=$this->Sys_model->db_get_row("A1010","*","F1028 like '%$barcode_auto' OR F1013 like '%$telaio_auto%'");
        }
        else
        {
            $row=$this->Sys_model->db_get_row("A1010","*","F1028 like '%$barcode_auto'");
        ...
        if($row!=null)
        {
            $return['modello']=$row['F1012'];
            $return['telaio']=$row['F1013'];
        }
        $return_json=json_encode($return);
        echo $return_json;
    }
    
    public function winteler_caricabarcode_test()
    {
        $post=$_POST;
        $files=$_FILES['file'];
        $filename=rand(100,1000).".jpg";
        if(count($files)>0)
        {
            move_uploaded_file($files['tmp_name'][0], "../AdiServer/barcode_test/$filename");
        }
        $command='"D:\Adiuto\ZBar\bin\zbarimg.exe" --raw -q D:\xampp\htdocs\AdiServer\barcode_test\\'.$filename.'"';
        exec($command,$output);
        $barcode="Barcode non riconosciuto";
        if(count($output)>0)
        {
        ...
        $return['barcodeold1']=$barcode;
        $command='"./tools/bardecode/bardecode.exe" "..\\AdiServer\\barcode_test\\'.$filename.'" -K 1RmK2jEXcERMF6F71C0g921q6H1RmKLM';
        exec($command,$output);
        $barcode="Barcode non riconosciuto";
        if(count($output)>0)
        {
        ...
        $return['barcodenew']=$barcode;
        $return['filename']=$filename;
        $return_json=json_encode($return);
        echo $return_json;
    }
    
    public function winteler_caricabarcodeOLD()
    {
        $post=$_POST;
        $files=$_FILES['file'];
        $filename=rand(100,1000).".jpg";
    ...
    public function winteler_barcode_quagga($filename)
    {
        $data['filename']=$filename;
        echo $this->load->view("sys/desktop/custom/winteler/winteler_barcode_quagga",$data,true);
    }
    
    public function winteler_conferma_prova($adiwid='')
    {
        $sql="UPDATE A1019 SET F1154='si' WHERE F1144='$adiwid'";
        $this->Sys_model->execute_query($sql);
        $data=array();
        $provaauto=$this->Sys_model->db_get_row("A1019","*","F1144='$adiwid'");
        if($this->isnotempty($provaauto))
        {
            $data['modello']=$provaauto['F1012'];
            $data['targa']=$provaauto['F1014'];
        ...
        echo $this->load->view("sys/desktop/custom/winteler/winteler_conferma_prova",$data,true);
    }
    
    public function winteler_conferma_serviceman($adiwid='')
    {
        $sql="UPDATE A1023 SET F1154='si' WHERE F1144='$adiwid'";
        $this->Sys_model->execute_query($sql);
        $data=array();
        
        $FIDD= $this->Sys_model->db_get_value("A1023","FIDD","F1144='$adiwid'");
        $allegati=$this->Sys_model->db_get("TLFD","*","FIDD='$FIDD'");
        $command_array=array();
        $allegati_copiati=array();
        foreach ($allegati as $key => $allegato) {
            $FPTH=$allegato['FPTH'];
            $FPTH= str_replace("/", "\\", $FPTH);
            $command='copy  "D:\Adiuto\\FTP'.$FPTH.'" "..\\AdiServer\\allegati\\'.$FIDD.'_'.$key.'.jpg"';
            $command_array[]=$command;
            $allegati_copiati[]=$FIDD."_".$key;
            exec($command);
        }
        $data['command_array']=$command_array;
        $data['allegati']=$allegati_copiati;
        echo $this->load->view("sys/desktop/custom/winteler/winteler_conferma_serviceman",$data,true);
    }
    
    
    
    
    public function salva_log($utente,$testo)
    {
        $today=date('Y-m-d H:i');
        $testo= str_replace("'", "''", $testo);
        $sql="
            INSERT INTO T_LOG (data,utente,testo)
            VALUES
            ('$today','$utente','$testo')
            ";
        $this->Sys_model->execute_query($sql);
    ...
    public function winteler_salva_checklist()
    {
        $post=$_POST;
        $userid=$this->session->userdata('userid');
        $adiwid=time ( )."-".rand(1000,9999);
        ...
        $this->salva_log($userid,"winteler_salva_checklist-post-".$testo);
        
        

        $insert_fields=array();
        $sql_insert_fields="F1144adiwid";
        $sql_insert_values="'$adiwid'";
        foreach ($post as $key => $value) {
            $sql_insert_fields=$sql_insert_fields.",$key";
            $sql_insert_values=$sql_insert_values.",'$value'";
        }
    
        $sql="
        INSERT INTO T_CHECKLIST
        ($sql_insert_fields)
        VALUES
        ($sql_insert_values)
        ";
        $this->salva_log($userid,"winteler_salva_checklist-sql-".$sql);
        $this->Sys_model->execute_query($sql);
        $files=$_FILES;
        ...
        if(array_key_exists('files', $_FILES))
        {
            ...
            if(count($files)>0)
            {
                    ...
                    foreach ($files['tmp_name'] as $key => $file) {
                        $filename="C#$adiwid#$counter.jpg";
                        move_uploaded_file($file, "../AdiServer/checklist/$filename");
                        $width=500;
                        $height=500;
                        $command='cd ./tools/ImageMagick/ && convert "../../../AdiServer/checklist/'.$filename.'" -resize '.$width.'x'.$height.' "../../../AdiServer/checklist/'.$filename.'"';
                        $this->esegui($command);
                        $command='move "..\\AdiServer\\checklist\\'.$filename.'" "D:\Adiuto\immission\Checklist"';
                        $this->esegui($command);
                        $counter++;
                    }
    ...
    public function winteler_load_test_venditori()
    {
        $data=array();
        $data['venditori']=$this->winteler_get_users_sales();
        $content= $this->load->view("sys/desktop/custom/winteler/winteler_test_venditori",$data,true);
        echo $content;
    }
    
    
    public function taleda_load_scan()
    {
        $data=array();
        echo $this->load->view("sys/desktop/custom/taleda/scan",$data,true);
    }
    ...
    {
        $data['img_oggetto']=$img_oggetto;
        $data['img_documento']=$img_documento;
        return $this->load->view("sys/desktop/custom/taleda/scan_combined",$data,true);
    }
    
    
    public function connect_mdb_old()
            {
        $conn=false;
    ...
    try {
        ...
        //$conn = new PDO("odbc:testmdb321", "", "");
$db_path='C:\Adiuto\Scansioni\Acquisto digitali immagine\20220721\Export.mdb';
    ...
    {
        $command='Xcopy /E /I "S:\\Adiuto\\Acquisto digitali immagine" "C:\\Adiuto\\xampp\\htdocs\\JDocServer\\custom\\taleda\\idscanner\\caricamento\\" '; 
        //$command='Xcopy /E /I "C:\\Adiuto\\xampp\\htdocs\\JDocServer\\custom\\taleda\\idscanner\\caricamentoBAK\\" "C:\\Adiuto\\xampp\\htdocs\\JDocServer\\custom\\taleda\\idscanner\\caricamento\\" '; 
        echo $command;
        exec($command);
    }
...
// iterate
    ...
    public function save_mdb_to_sqlserver()
    {
        $dati=$this->get_mdb();
        var_dump($dati);
